...
The most important package is a chrome (derivative) able to render HTML to PNG images.
InkyPi will automatically pick from a list of well-known [candidates](../src/utils/image_utils.py) from the `PATH`.

### Linux

#### Debian and Derivatives

The required packages can be found in this [file](../install/debian-requirements.txt).
Use your favourite package manager (such as `apt`) to install them.

#### Other Distributions

Adapt the list of packages from [debian](../install/debian-requirements.txt) to your own distribution.

### Windows & macOS

The most important package is `chromium-headless-shell`, which is used to render HTML templates to PNG images.

Chromium Headless Shell doesn't appear to be available for Windows or macOS, but the alternative Chrome Headless Shell is.
Download a suitable release of `chrome-headless-shell` for your OS (i.e. win64 = Windows 11 64-bit, or mac-arm64 = Apple Silicon Mac) from here:

https://googlechromelabs.github.io/chrome-for-testing/

Unzip 'chrome-headless-shell-win64.zip', for example in the same parent folder as InkPi:
...
Make sure the binary `chrome-headless-shell.exe` or `chrome-headless-shell` is available in your `PATH`.
