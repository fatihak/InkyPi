...
# Default GPIO pins (Inky Impression layout)
DEFAULT_BUTTON_PINS = {
    ButtonID.A: 5,
    ButtonID.B: 6,
    ButtonID.C: 16,
...
class GPIOButtonHandler(AbstractButtonHandler):
    """
    GPIO button handler for Raspberry Pi.
    
    Uses gpiozero library for GPIO pin access.
    Supports short, double, and long press detection.
    Default pins are configured for Inky Impression, but can be customized.
    """
    ...
    def start(self):
        ...
        try:
            ...
            self._running = True
            logger.info("GPIOButtonHandler started")
    ...
    def stop(self):
        ...
        self._running = False
        logger.info("GPIOButtonHandler stopped")
